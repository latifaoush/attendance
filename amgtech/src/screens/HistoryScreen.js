    ...
    >
      ...
      <View
        className={`w-1 h-10 rounded-full mr-3.5 ${
          highlight ? 'bg-red-400' : type === 'Masuk' ? 'bg-gray-400' : 'bg-gray-300'
        }`}
      />
      ...
      {!loading && !error && groupedData.length > 0 && (
        ...
        >
          {groupedData.map((group, gi) => (
            <View key={gi} className="flex-row mb-5">
              <View
              ...
              >
                <View className="flex-row">
                  ...
                  <View className="w-[68px] items-center mr-4 pt-0.5">
                    <View
                      className="w-[60px] items-center bg-gray-700 rounded-2xl py-2.5"
                    >
