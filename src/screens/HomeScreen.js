...
export default function HomeScreen({ setToken }) {
  ...
  const registerDisabled = user?.total_face < 5;
  const buttonDisabled =
    checkInDisabled || checkOutDisabled || registerDisabled;
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      {/* ── Bottom CTA Buttons ── */}
      {user?.statusregister == '1' && user?.total_face < 5 && (
        <TouchableOpacity
        ...
        >
          ...
          >
            Registrasi Wajah ({user?.total_face || 0}/5)
          </Text>
        </TouchableOpacity>
      )}
