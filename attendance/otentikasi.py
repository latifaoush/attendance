...
def save_mysql_new(filename, face_enc, nip):
    ...
    connection = pymysql.connect(host='94.237.2.166:3360', database='face', user='root',
                                 password='Gates12345!!!123!@#', charset='utf8', use_unicode=True)
    #connection = pymysql.connect('94.237.2.166:3360', 'root', '', 'face')
...
def upload_image():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            # nip = file.filename.replace(".jpg", "")
            return detect_faces_in_mysql(nip, file)
...
def detect_faces_in_compare(nip, file_stream):
    ...
    connection = pymysql.connect(host='94.237.2.166:3360', database='face', user='root',
                                 password='Gates12345!!!123!@#', charset='utf8', use_unicode=True)
    #'94.237.2.166:3360', 'root', 'Gates12345!!!123!@#', 'face'
...
def detect_faces_in_mysql(nip, file_stream):
    ...
    train_dir = os.listdir(os.path.join(app.instance_path, 'uploads/'))
    # Select what we just added
    #connection = pymysql.connect('94.237.2.166:3360', 'root', 'Gates12345!!!123!@#', 'face')
    connection = pymysql.connect(host='94.237.2.166:3360', database='face', user='root',
                                 password='Gates12345!!!123!@#', charset='utf8', use_unicode=True)
    cursorparent = connection.cursor()
    cursor = connection.cursor()
